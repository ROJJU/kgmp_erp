...
<%@ page import="kgmp.common.beans.Report, java.util.Map, java.util.List, java.util.*" %>
...
<html>
	<head>
	    ...
	    <link rel="stylesheet" type="text/css" href="css/Daily_R.css"/>
					...
					<center>
						<table class="daily">
							...
							<tr>
								<th>전달</th>
								<td  width="200px" >
									<c:if test="${d.send0!=null}"><img src="img/person.png" width="5px" title="${d.send0}"></c:if>
									<c:if test="${d.send1!=null}"><img src="img/person.png" width="5px" title="${d.send1}"></c:if>
									<c:if test="${d.send2!=null}"><img src="img/person.png" width="5px" title="${d.send2}"></c:if>
									<c:if test="${d.send3!=null}"><img src="img/person.png" width="5px" title="${d.send3}"></c:if>
									<c:if test="${d.send4!=null}"><img src="img/person.png" width="5px" title="${d.send4}"></c:if>	
								</td>
								...
								<form action ="Daily_commit_proc.jsp" method="post" name="submit">
									<td align="center"  colspan="3">
										<c:if test="${d.state==0}">
											<c:if test="${member_id==d.id}"><select name="submit"><option value="2">보완</option><option value="1">컴펌 완료</option></select>&nbsp; <input type="image" src="img/condition.png" width="56px" onClick="formCheck()"/></c:if>
							...
							<tr>
								...
								<td colspan="7">
									<form action="Download_d.jsp" method="post" name="down">
										<a href="javaScript:document.down.submit();" id="d_file">${d.d_file}</a>
						...
						<div id="bottonPlace">
							<div id="bottons">
							<%if(day_s==0){%>
								<input type="image" src="../notice/img/list.png" width="50px" onclick="goUrl('DailyView.jsp?year_m=<%=year_m%>&month_m=<%=month_m%>&year_g=<%=year_g%>&month_g=<%=month_g%>&day_g=<%=day_g%>')">
							<%}else if(day_g==0){%>
								<input type="image" src="../notice/img/list.png" width="50px" onclick="goUrl('DailyView.jsp?year_m=<%=year_m%>&month_m=<%=month_m%>&year_s=<%=year_s%>&month_s=<%=month_s%>&day_s=<%=day_s%>')">
							<%}%>
								<c:if test="${d.state==1||d.state==0}"><input type="image" src="img/supplement.png" width="74px" onClick="goUrl('Daily_reply_add.jsp?ref_seq=${d.ref_seq}&step=${d.step}&depth=${d.depth}&daily_seq=<%=daily_seq%>')"></c:if>
								<c:if test="${d.state==-1}"><input type="image" src="img/submit.png" width="72px" onclick="javaScript:alert('컴펌완료된 서류는 보완 제출할 수 없습니다.')"></c:if>
							</div>
						</div>
						...
						<c:forEach var="a" items="${list}">
							<tr>
								...
								<td align="center" id="menu1${a.d_comment_seq}"><input type="image" src="img/link.png" width="10px"></td>
							</tr>
<!--hide 된 tr START!!-->
							...
							<form action="Daily_comment_reply_add_proc.jsp" method="post" name="comment_reply">
								...
								<td align="center"><input type="image" src="img/Comment.png" width="40px" onclick="javaScript:document.comment_reply.submit();"></td>
								<td id="bye${a.d_comment_seq}" align="center">
									<input type="image" src="img/icon_x.png" width="10px"/>
									<input type="hidden" value="<%=daily_seq%>" name="daily_seq">
							...
							<form action="Daily_comment_add_proc.jsp" method="post" name="comment" >
								...
								<td align="center">
									...
									<%}else if(day_g==0){%>
										...
										<input type="hidden" value="<%=day_s%>" name="day_s">	
									<%}%>
									<input type="image" src="img/Comment.png" width="40px;" onclick="javaScript:document.comment_reply.submit();">
									<input type="hidden" value="${member_name}" name ="c_writer">
									<input type="hidden" value="<%=daily_seq%>" name="daily_seq">
								</td>
							</form>
