...
//현재페이지 설정
	...
	if(request.getParameter("day_g") != null){
		if(request.getParameter("day_g").equals("0")){
			day_g=nowD;
		}else{
...
<html>
	<head>
	    ...
	    <link rel="stylesheet" type="text/css" href="css/Daily_V.css"/>
	...
	<body onload="window_onload()">
		...
		<jsp:include page="../main/SideMenu.jsp"/>
			<div id="content">
				<div id = "titlePlace">
					<div id ="pageTitle"><b>DailyReport</b></div>
					<div id ="dailyAdd"><input type="image" src="../schedule/img/link.png" width="11px" onclick="goUrl('DailyAdd.jsp')"></div>
				</div>
				<div class="box1">
					<div id = "font1">
						  <ul class="main_menu1">
							   <li class="active">월별 확인</li>
						  </ul>
					 </div>
				</div><br>
<!--받은 업무 start!!!-->
				<div class="box2">
					<table>
						<tr>
							<td colspan="32">
								<a href="DailyView.jsp?year_m=<%= month_m-1== 0? year_m-1:year_m%>&month_m=<%=month_m-1==0? 12:month_m-1%>&id=<%=id%>"><input type="image" src="../main/images/but-pre.png"></a>
								현재 <b><%=id%></b>님의 <font color="red"><%=year_m%></font>년 <font color="red"><%=month_m%></font>월 업무 내역 입니다.
								<a><input type="image" src="../main/images/but-next.png" onclick="goUrl('DailyView.jsp?year_m=<%= month_m+1==13?year_m+1:year_m%>&month_m=<%=month_m+1==13?1:month_m+1 %>&id=<%=id%>')"></a>
							</td>
						</tr>
						<tr>
							<td align="right" colspan="32">
								<form action="DailyView.jsp" method="get" name="date_m">
						...
						<tr class="a">
							...
							<%for(int s=1;s<=31;s++){
								...
								if(count!=0){
							%>
							<td align="center" class="selectTD" onclick="goUrl('DailyView.jsp?year_g=<%=year_m%>&month_g=<%=month_m%>&day_g=<%=s%>&year_m=<%=year_m%>&month_m=<%=month_m%>&id=<%=id%>')"><b><font color="red"><%=count %></font></b></td>
							<%}else{%>
							<td align="center" class="selectTD" onclick="goUrl('DailyView.jsp?year_g=<%=year_m%>&month_g=<%=month_m%>&day_g=<%=s%>&year_m=<%=year_m%>&month_m=<%=month_m%>&id=<%=id%>')"><%=count %></td>
							<%}}%>
						</tr>
						<tr class="a">
							...
							<%for(int s=1;s<=31;s++){
								...
								if(count!=0){
							%>
							<td align="center" class="selectTD" onclick="goUrl('DailyView.jsp?year_s=<%=year_m%>&month_s=<%=month_m%>&day_s=<%=s%>&year_m=<%=year_m%>&month_m=<%=month_m%>&id_s=<%=id%>')"><b><font color="blue"><%=count %></font></b></td>
							<%}else{%>
							<td align="center" class="selectTD" onclick="goUrl('DailyView.jsp?year_s=<%=year_m%>&month_s=<%=month_m%>&day_s=<%=s%>&year_m=<%=year_m%>&month_m=<%=month_m%>&id_s=<%=id%>')"><%=count %></td>
							<%}}%>
						</tr>
					</table>
				</div><br>
				...
				<div id="serchPlace">
					<form action="DailyView.jsp" method="post" name="get">
						<div id ="serch">
					...
					<form action="DailyView.jsp" method="post" name="date_g">
						<div id="page">
							${pageList_g}&nbsp;
							(일별보기 : <select name="year_g" id="syear_g"><option></option></select><select name="month_g" id="smonth_g"><option></option></select><select name="day_g" id="sday_g"><option></option></select>)
							 <a href="javaScript:document.date_g.submit();" title="받은 업무 일별보기">GO</a>
							  / <a href="DailyView.jsp?year_g=<%=nowY%>&month_g=<%=nowM%>&day_g=<%=nowD%>" title="받은 업무 <%=nowY%>년<%=nowM%>월<%=nowD%>일 이동">Today</a>
						</div>
					</form>
				</div>
				<div id = "bodyTable">
					<center>
						...
						<c:forEach var="a" items="${list_g}"><%i++;%>
							<tr onclick="goUrl('DailyRead.jsp?daily_seq=${a.daily_seq}&year_m=<%=year_m%>&month_m=<%=month_m%>&id=<%=id%>&year_g=<%=year_m%>&month_g=<%=month_m%>&day_g=<%=day_g%>')" class="a">
								...
								<c:if test="${a.d_file!=null}"><td align="center"><input type="image" src="img/icon_attach.gif" title="${a.d_file}"></td></c:if>
...
<!--전달한업무 start!!-->
						...
						<div id="serchPlace">
							<form action="DailyView.jsp" method="post" name="send">
								<div id ="serch2">
							...
							<form action="DailyView.jsp" method="post" name="date_s">
								<div id="page2">
									${pageList_s}&nbsp;
									(일별보기 : <select name="year_s" id="syear_s"><option></option></select><select name="month_s" id="smonth_s"><option></option></select><select name="day_s" id="sday_s"><option></option></select>)
									 <a href="javaScript:document.date_s.submit();" title="전달업무 일별보기">GO</a>
									   / <a href="DailyView.jsp?year_s=<%=nowY%>&month_s=<%=nowM%>&day_s=<%=nowD%>"  title="전달 업무 <%=nowY%>년<%=nowM%>월<%=nowD%>일 이동">Today</a>
								</div>
							</form>
						</div>
					<center>
						...
						<c:forEach var="b" items="${list_s}"><%j++;%>
							<tr onclick="goUrl('DailyRead.jsp?daily_seq=${b.daily_seq}&year_m=<%=year_m%>&month_m=<%=month_m%>&id=<%=id%>&year_s=<%=year_m%>&month_s=<%=month_m%>&day_s=<%=day_s%>')" class="a">
								...
								<c:if test="${b.d_file!=null}"><td align="center"><input type="image" src="img/icon_attach.gif" title="${b.d_file}"></td></c:if>
								...
								<td align="center" >
									<c:if test="${b.send0!=null}"><img src="img/person.png" width="5px" title="${b.send0}"></c:if>
									<c:if test="${b.send1!=null}"><img src="img/person.png" width="5px" title="${b.send1}"></c:if>
									<c:if test="${b.send2!=null}"><img src="img/person.png" width="5px" title="${b.send2}"></c:if>
									<c:if test="${b.send3!=null}"><img src="img/person.png" width="5px" title="${b.send3}"></c:if>
									<c:if test="${b.send4!=null}"><img src="img/person.png" width="5px" title="${b.send4}"></c:if>	
								</td>
