...
String check = report_DAO.insertComment(report);

if(check.equals("true")){
	%>
			alert('코맨트를 저장 하였습니다.');
			<%if(day_s==0){%>
			location.href="DailyRead.jsp?daily_seq=<%=report.getDaily_seq()%>&year_m=<%=year_m%>&month_m=<%=month_m%>&year_g=<%=year_g%>&month_g=<%=month_g%>&day_g=<%=day_g%>&id=<%=report.getId()%>";
			<%}else if(day_g==0){%>
			location.href="DailyRead.jsp?daily_seq=<%=report.getDaily_seq()%>&year_m=<%=year_m%>&month_m=<%=month_m%>&year_s=<%=year_s%>&month_s=<%=month_s%>&day_s=<%=day_s%>&id=<%=report.getId()%>";
			<%}%>
	<%
}else{
	%>
